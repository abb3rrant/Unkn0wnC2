...
func main() {
	...
	go func() {
		...
		for {
			...
			// NOTE: We intentionally do NOT cleanup beacons or their tasks.
			// Beacon records should be preserved indefinitely for:
			// 1. Historical reference (when did beacon first/last check in)
			// 2. Task history (what commands were executed)
			// 3. Potential reconnection (beacon may come back online)
			...
			// Requeue tasks stuck in 'sent' with no result chunks (beacon died post-delivery)
			if count, err := db.RequeueSentTasks(30); err == nil && count > 0 {
				LogInfo("Cleanup: Requeued %d tasks stuck in 'sent' with no result (>30 min)", count)
			} else if err != nil {
				LogWarn("Cleanup: Failed to requeue sent tasks: %v", err)
			}
