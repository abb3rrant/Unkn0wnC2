...
// writeDNSServerLog appends forwarded DNS server log entries to a per-server log file.
func writeDNSServerLog(serverID string, entries []struct {
	...
	logPath := filepath.Join(logDir, fmt.Sprintf("dns-%s.log", serverID))
