...
pub struct Config {
    ...
    pub max_subdomain_length: usize, // 0 = default (253 - domain_len)
    pub encrypted: bool,             // false = plain base36, no AES-GCM
}
...
fn tune_chunk_bytes(mut cfg: Config) -> Config {
    let max_allowed = max_supported_chunk_bytes(&cfg.domains, cfg.max_subdomain_length, cfg.encrypted);
    let original = cfg.chunk_bytes;

    if max_allowed == 0 {
        cfg.chunk_bytes = 1;
        let _ = ADJUSTMENT.set(Some((original, 1)));
        return cfg;
    }
...
impl From<FileConfig> for Config {
    fn from(value: FileConfig) -> Self {
        ...
        Config {
            ...
            max_subdomain_length: value.max_subdomain_length,
...
const fn default_encrypted() -> bool {
    true
}
