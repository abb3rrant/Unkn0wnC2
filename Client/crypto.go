...
// base36EncodeString encodes a plaintext string as base36 with no encryption.
// Used by the "base36" (unencrypted) beacon encoding mode.
func base36EncodeString(data string) string {
	return base36Encode([]byte(data))
}

// base36DecodeString decodes a plain base36 string back to a string.
func base36DecodeString(encoded string) (string, error) {
	b, err := base36Decode(encoded)
	if err != nil {
		return "", err
	}
	return string(b), nil
