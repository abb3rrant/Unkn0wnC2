...
const (
	...
	// UnixTimestampMinLength is the minimum length of a timestamp label.
	// Stagers now send last-5-digits of Unix time (%05d); old stagers sent the full
	// 10-digit value. 5 is the new minimum.
	UnixTimestampMinLength = 5

	// UnixTimestampMaxLength is the maximum length of Unix timestamps
	UnixTimestampMaxLength = 11
)

// Map Size Bounds (prevents unbounded memory growth)
const (
	// MaxRecentMessages is the maximum number of dedup message hashes to keep
	MaxRecentMessages = 10000

	// MaxCompletedStagerLogs is the maximum number of completed stager session IDs to track
	MaxCompletedStagerLogs = 5000

	// MaxTasksInProgress is the maximum number of in-progress task entries
	MaxTasksInProgress = 2000

	// MaxMasterTaskIDs is the maximum number of master task ID mappings
	MaxMasterTaskIDs = 5000
)
