...
// LogEntry represents a single log entry for forwarding
...
// If logDir is empty, uses the directory containing the server binary
// debugMode: if true, also output to stdout
func InitLogger(logDir string, debugMode bool) error {
	var initErr error
	logOnce.Do(func() {
		// Use the binary's directory if not specified
		if logDir == "" {
...
// DrainLogBuffer returns and clears all buffered log entries
func DrainLogBuffer() []LogEntry {
	if serverLogger == nil {
		return nil
	}
	serverLogger.bufferMu.Lock()
	defer serverLogger.bufferMu.Unlock()

	if len(serverLogger.logBuffer) == 0 {
		return nil
	}

	entries := serverLogger.logBuffer
	serverLogger.logBuffer = nil
	return entries
}
