...
func parseLabelEncodedExfilFrame(qname string, domains []string, aesKey []byte) (*ExfilFrame, bool, error) {
	...
	for _, domain := range domains {
		...
		// Only treat as exfil if first label has the EX prefix
		// Non-exfil traffic (beacon CHK, etc.) should fall through to C2 processing
		if !strings.HasPrefix(strings.ToUpper(frameLabels[0]), ExfilMetadataPrefix) {
			continue
		}
...
func interpretEncryptedFrameLabels(labels []string, aesKey []byte) (*ExfilFrame, error) {
	...
	// Try AES-GCM decryption first, then fall back to plain base36 (unencrypted exfil clients)
	encrypted := true
	plaintext, err := decodeAndDecryptBytes(encoded, aesKey)
	if err != nil {
		plainBytes, b36Err := base36Decode(strings.ToLower(encoded))
		if b36Err != nil || len(plainBytes) == 0 {
			return nil, fmt.Errorf("failed to decrypt metadata label (AES: %w; plain base36 also failed)", err)
		}
