...
func (d *Database) migration3ExfilSync() error {
	...
	_, err := d.db.Exec(`ALTER TABLE exfil_chunks ADD COLUMN synced INTEGER DEFAULT 0`)
	if err != nil {
		return err
	}
	// Create index in separate Exec (SQLite drivers may not support multi-statement)
	_, err = d.db.Exec(`CREATE INDEX IF NOT EXISTS idx_exfil_chunks_synced ON exfil_chunks(synced)`)
	return err
}
...
func (d *Database) CacheStagerChunks(clientBinaryID string, chunks []string) error {
	...
	// Remove old binary chunks before inserting new ones
	if _, err := tx.Exec("DELETE FROM stager_chunk_cache WHERE client_binary_id != ?", clientBinaryID); err != nil {
		return err
	}
